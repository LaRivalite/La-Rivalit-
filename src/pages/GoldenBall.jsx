...
export default function GoldenBall() {
  ...
  useEffect(() => {
    async function load() {
      const result = await fetchStatsData("gold");
      setData(result);
      setLoading(false);
    }
    load();
    ...
    const filtered = data.deliveries.filter(d => {
      ...
      if (selectedOverRanges.length > 0) {
        ...
        const inRange = selectedOverRanges.some(range => {
          ...
          if (range === "middle")    return over >= 7 && over <= 15;
  ...
  const battingStats  = useMemo(() => {
    const all = data ? computeBattingStats(filteredDeliveries, data.playersById) : [];
    return teamFilter === 'all' ? all : all.filter(p => p.team === teamFilter);
  }, [filteredDeliveries, data, teamFilter]);

  const bowlingStats  = useMemo(() => {
    const all = data ? computeBowlingStats(filteredDeliveries, data.playersById) : [];
    return teamFilter === 'all' ? all : all.filter(p => p.team === teamFilter);
  }, [filteredDeliveries, data, teamFilter]);

  const fieldingStats = useMemo(() => {
    const all = data ? computeFieldingStats(filteredDeliveries, data.playersById) : [];
    return teamFilter === 'all' ? all : all.filter(p => p.team === teamFilter);
  }, [filteredDeliveries, data, teamFilter]);
  ...
  const h2hResult = useMemo(() => {
    ...
    return computeFullH2H(filteredDeliveries, h2hBatter, h2hBowler, data.playersById);
  ...
  return (
    <div className="px-4 lg:px-20 py-10 max-w-6xl mx-auto">
      ...
      <div className="text-center mb-10">
        ...
        <h1 className="text-4xl sm:text-5xl font-black uppercase tracking-[0.15em]">GOLDEN BALL</h1>
        <p className="mt-3 text-zinc-500">Golden Ball • Stats & Records</p>
      </div>
    ...
    <div className="space-y-3">
      {filtered.map((p, i) => {
        ...
        return (
          <button
            key={p.id}
            onClick={() => navigate(`/player/gold/${p.id}`)}
  ...
  return (
    <div className="overflow-x-auto -mx-2">
      <table className="w-full text-sm min-w-[560px]">
        ...
        <tbody>
          {stats.map(p => (
            <tr key={p.id} onClick={() => navigate(`/player/gold/${p.id}`)}
